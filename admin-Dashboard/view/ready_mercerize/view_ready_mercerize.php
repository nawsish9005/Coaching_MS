                          ...
                          <table class="table table-striped table-bordered">
                            ...
                            <tbody>
                              <?php
                                ...
                                {
                                    $ready_mercerize_id = $row_pp_details['ready_mercerize_id'];
                                ...
                                ?>
                                  <tr>
                                    ...
                                    <td><?php echo $row_pp_details['bowing_and_skew'] ?></td>
                                    <td><?php echo $row_pp_details['ph_left'] ?></td>
                                    <td><?php echo $row_pp_details['ph_center'] ?></td>
                                    <td><?php echo $row_pp_details['ph_right'] ?></td>
                                    <td>
                                        <input type="hidden" id="route_issue_id<?php echo $ready_mercerize_id; ?>" name="route_issue_id<?php echo $ready_mercerize_id; ?>" value="<?php echo $route_issue_id; ?>">
                                        <input type="hidden" id="greige_issunce_id<?php echo $ready_mercerize_id; ?>" name="greige_issunce_id<?php echo $ready_mercerize_id; ?>" value="<?php echo $greige_issunce_id; ?>">
                                        <input type="hidden" id="pp_no_id<?php echo $ready_mercerize_id; ?>" name="pp_no_id<?php echo $ready_mercerize_id; ?>" value="<?php echo $pp_no_id; ?>">
                                        <input type="hidden" id="pp_details_id<?php echo $ready_mercerize_id; ?>" name="pp_details_id<?php echo $ready_mercerize_id; ?>" value="<?php echo $pp_details_id; ?>">
                                        <button type="button" id="gray_issue_add_btn" value="<?php echo $ready_mercerize_id; ?>" name="gray_issue_add_btn" onclick="ready_mercerize_view(this.value);" class="btn btn-primary btn-xs"> Action </button>
                                    </td>
                                  </tr>
                                <?php 
                                  ++$sl;
...
<script type="text/javascript">
  function ready_mercerize_view(ready_mercerize_id)
  {
      var route_issue_id = document.getElementById("route_issue_id"+ready_mercerize_id).value;
      var greige_issunce_id = document.getElementById("greige_issunce_id"+ready_mercerize_id).value;
      var pp_no_id = document.getElementById("pp_no_id"+ready_mercerize_id).value;
      var pp_details_id = document.getElementById("pp_details_id"+ready_mercerize_id).value;

      $.ajax(
      {
          type: "POST",
          url: "details_ready_mercerize_view.php",
          method: "POST",
          data: {route_issue_id: route_issue_id, greige_issunce_id: greige_issunce_id, pp_no_id: pp_no_id, pp_details_id: pp_details_id, ready_mercerize_id: ready_mercerize_id},
