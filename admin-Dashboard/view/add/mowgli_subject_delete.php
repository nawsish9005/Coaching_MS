...
$select_sql_statement = "SELECT * FROM `m_courses` WHERE m_courses_id = $m_courses_id";
$result_sql_statement = mysqli_query($con, $select_sql_statement);
$row_sql_statement = mysqli_fetch_assoc($result_sql_statement);
$image_src_of_id = $row_sql_statement["courses_image"];
$image_src_loc_of_id = "../../build/user_pic/".$row_sql_statement["courses_image"];

mysqli_query($con, "BEGIN");
mysqli_query($con, "START TRANSACTION") or die(mysqli_error($con));

$delete_sql_statement = "DELETE FROM `m_courses` 
						  WHERE m_courses_id = '$m_courses_id'";
