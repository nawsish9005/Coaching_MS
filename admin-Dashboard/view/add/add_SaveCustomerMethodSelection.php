<?php 
	...
	$customers_name=	$demo2 = mysqli_real_escape_string($con, stripslashes(trim($_POST['customer'])));
...
if (($customers_name == '') || (empty($customers_name)) || is_null($customers_name)) 
   ...
   $delete_sql_statement1 ="DELETE FROM `customer_selection` WHERE `customer_name`='$customers_name'";
   mysqli_query($con, $delete_sql_statement1) or die(mysqli_error($con));
foreach($demo3 as $x => $val) {
 $x = $val;

 $sql_for_duplicate = "SELECT * FROM `customer_selection` WHERE `customer_name`='$customers_name' and `test_method_id`='$val'"; 
