<?php 
	...
	$ready_for_raising_id = mysqli_real_escape_string($con, stripslashes(trim($_POST['ready_for_raising_id'])));
    ...
    $ready_for_raising_quality = mysqli_real_escape_string($con, stripslashes(trim($_POST['ready_for_raising_quality'])));
	...
	$insert_pp_details_statement = "UPDATE `ready_for_raising` 
								    SET `date`= '$date',
								   		`b_batcher`= '$b_batcher',
								   		`a_batcher`= '$a_batcher',
								   		`p_width`= '$p_width',
								  ...
								  WHERE `route_issue_id` = '$route_issue_id' AND ready_for_raising_id='$ready_for_raising_id' ";
