...
if (($greige_issunce_id == '') || (empty($greige_issunce_id)) || is_null($greige_issunce_id)||
  ($route_issue_id == '') || (empty($route_issue_id)) || is_null($route_issue_id)) {
   header("Location: ready_for_raising_list.php");
}
?>
                          ...
                          <table class="table table-striped table-bordered">
                            <thead>
                              <tr>
                                ...
                                <th>TRF No.</th>
                                <th>Tensile Properties warp</th>
                                <th>Tensile Properties weft</th>
                                <th>Tear Force warp</th>
                                <th>Tear Force weft</th>
                            ...
                            <tbody>
                              <?php
                                ...
                                {
                                    $ready_for_raising_id = $row_pp_details['ready_for_raising_id'];
                                ...
                                ?>
                                  <tr>
                                    ...
                                    <td><?php echo $row_pp_details['trf'] ?></td>
                                    <td><?php echo $row_pp_details['tensile_warp'] ?></td>
                                    <td><?php echo $row_pp_details['tensile_weft'] ?></td>
                                    <td><?php echo $row_pp_details['tear_force_warp'] ?></td>
                                    <td><?php echo $row_pp_details['tear_force_weft'] ?></td>
                                

                                    <td>
                                        <input type="hidden" id="route_issue_id<?php echo $ready_for_raising_id; ?>" name="route_issue_id<?php echo $ready_for_raising_id; ?>" value="<?php echo $route_issue_id; ?>">
                                        <input type="hidden" id="greige_issunce_id<?php echo $ready_for_raising_id; ?>" name="greige_issunce_id<?php echo $ready_for_raising_id; ?>" value="<?php echo $greige_issunce_id; ?>">
                                        <input type="hidden" id="pp_no_id<?php echo $ready_for_raising_id; ?>" name="pp_no_id<?php echo $ready_for_raising_id; ?>" value="<?php echo $pp_no_id; ?>">
                                        <input type="hidden" id="pp_details_id<?php echo $ready_for_raising_id; ?>" name="pp_details_id<?php echo $ready_for_raising_id; ?>" value="<?php echo $pp_details_id; ?>">
                                        <button type="button" id="gray_issue_add_btn" value="<?php echo $ready_for_raising_id; ?>" name="gray_issue_add_btn" onclick="ready_for_raising_view(this.value);" class="btn btn-primary btn-xs"> Action </button>
                                    </td>
                                  </tr>
                                <?php 
                                  ++$sl;
...
<script type="text/javascript">
  function ready_for_raising_view(ready_for_raising_id)
  {
      var route_issue_id = document.getElementById("route_issue_id"+ready_for_raising_id).value;
      var greige_issunce_id = document.getElementById("greige_issunce_id"+ready_for_raising_id).value;
      var pp_no_id = document.getElementById("pp_no_id"+ready_for_raising_id).value;
      var pp_details_id = document.getElementById("pp_details_id"+ready_for_raising_id).value;

      $.ajax(
      {
          type: "POST",
          url: "details_ready_for_raising_view.php",
          method: "POST",
          data: {route_issue_id: route_issue_id, greige_issunce_id: greige_issunce_id, pp_no_id: pp_no_id, pp_details_id: pp_details_id, ready_for_raising_id: ready_for_raising_id},
