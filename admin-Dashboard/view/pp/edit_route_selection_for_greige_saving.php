<?php 
	...
	$sql_for_row_data = " SELECT id FROM route_issue_main WHERE pp_no_id = '$pp_no_id' AND pp_details_id = '$pp_details_id' AND active = '1' ";
	...
	//for insert privious update data
 	for ($i = 1; $i <= $name_counter; $i++)
 	{
 		...
 		if($_POST['route'.$i] && $_POST['route_issue_main_id'.$i])
 		{
			...
			$process_number = mysqli_real_escape_string($con, stripslashes(trim($_POST['process_number'.$i])));
			$route_issue_main_id = mysqli_real_escape_string($con, stripslashes(trim($_POST['route_issue_main_id'.$i])));
