<?php 
	...
	if (($date == '') || (empty($date)) || is_null($date) || ($pp_no == '') || (empty($pp_no)) || is_null($pp_no) || ($greige_demand == '') 
		...
		|| ($construction == '') || (empty($construction)) || is_null($construction) || ($customer == '') || (empty($customer))
		|| is_null($customer) || ($process == '') || (empty($process)) || is_null($process) || ($counter == '') || (empty($counter)) || is_null($counter) || ($_POST['version1'] == '') ||
	...
	{ 
	 	for ($i=1; $i <= $name_counter; $i++)
	 	{
	 		...
	 		if($_POST['version'.$i])
	 		{
				...
				$construction = mysqli_real_escape_string($con, stripslashes(trim($_POST['construction'.$i])));
				$fiber_cotton = mysqli_real_escape_string($con, stripslashes(trim($_POST['fiber_cotton'.$i])));
				$fiber_polyster = mysqli_real_escape_string($con, stripslashes(trim($_POST['fiber_polyster'.$i])));
				$fiber_others = mysqli_real_escape_string($con, stripslashes(trim($_POST['fiber_others'.$i])));
