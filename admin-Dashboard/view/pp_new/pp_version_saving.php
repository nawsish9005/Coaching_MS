<?php 
	...
	$version = mysqli_real_escape_string($con, stripslashes(trim($_POST['version_name'])));
	$color = mysqli_real_escape_string($con, stripslashes(trim($_POST['color_name'])));

	$construction = mysqli_real_escape_string($con, stripslashes(trim($_POST['construction_for_version'])));
 ...
 // 	{
	...
	//check process
	$process_check_query = "SELECT * FROM process_technique_or_program_type WHERE process_id = '$process_line'";
	...
	{ 
		$sql = 'SELECT id FROM version_wise_process_program_info ORDER BY id DESC LIMIT 1';
		$sql_res = mysqli_query($con, $sql) or die(mysqli_error($con));
		$sql_row = mysqli_fetch_assoc($sql_res);
		$pp_id = $sql_row['id'] + 1;

		$insert_pp_details_statement = "INSERT INTO `version_wise_process_program_info` 
						                             ( 
						                              `pp_id`,
						                              `pp_no_id`, 
						                              `version`, 
