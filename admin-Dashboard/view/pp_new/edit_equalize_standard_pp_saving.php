<?php 
  ...
  $equalize_standard_id = mysqli_real_escape_string($con, stripslashes(trim($_POST['equalize_standard_id'])));
  ...
  $update_sql_statement = "UPDATE `defining_equalize_qc_standard` 
   ...
   { 
       ...
       $sql = 'SELECT id
                FROM defining_equalize_qc_standard ORDER BY id DESC LIMIT 1';
       $sql_res = mysqli_query($con, $sql) or die(mysqli_error($con));
       $sql_row = mysqli_fetch_assoc($sql_res);
       $equalize_standard_id = $sql_row['id'] + 1;

    	$insert_sql_statement = "INSERT INTO `defining_equalize_qc_standard` 
                                 ( 
